...
def clean_data(df):
    ...
    category_colnames = row.transform(lambda x: x[:-2])
    categories.columns = category_colnames
    
    for column in categories:
        categories[column] = categories[column].transform(lambda x: int(x[-1:]))
...
def save_data(df, database_filename):
    ...
    Arguments:
        df: Cleaned Messages and Categories data
        database_filename: Path to sqlite database, end with .db
    """
...
def main():
    """
    Main function kick off the data processing functions, including:
        1) load_data()
        2) clean_data()
        3) save_data()
    """
